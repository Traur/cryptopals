...
def estimateKeysize(ciphertext, maxlen):	
	assert type(ciphertext) == bytearray or type(ciphertext) == bytes
	#Possible distances go from 0 (exakt same) to maxlen*8 (every bit is different (0, maxlen*8]	
	...
	for keysize in range(2, maxlen):
		...
		#   proceed perhaps with the smallest 2-3 KEYSIZE values. Or take 4 KEYSIZE blocks instead of 2 and average the distances. 
		if distance < smallestDistance:
			print("Better keysize n={0} found witch d={1}".format(keysize, distance))
			smallestDistance = distance
			bestKeysize.append(keysize)
			
	return bestKeysize[::-1] #reverse list so shortest distance keylength has i=0